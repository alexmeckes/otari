...
from gateway.streaming import _combine_usage


def test_combine_usage_cumulative() -> None:
    chunks = [
    ...
    for chunk in chunks:
        result = _combine_usage(result, chunk)
...
def test_combine_usage_final_chunk_only() -> None:
    ...
    result = _combine_usage(base, final)
...
def test_combine_usage_preserves_current_on_zero_update() -> None:
    ...
    result = _combine_usage(current, update)
